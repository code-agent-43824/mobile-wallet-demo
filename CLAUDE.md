...
Flutter demo of a mobile EVM crypto wallet targeting Android, iOS, and Windows x64. It supports the full onboarding/auth shell, an encrypted on-device key vault, read-only blockchain access (Ethereum Mainnet + Sepolia), and end-to-end EIP-1559 signing/sending. Android v1.44 includes the first real Rutoken NFC/PKCS#11 transport and a read-only physical diagnostic. Its startup mirrors the official demo: the PC/SC bridge attaches from a custom `Application.onCreate` before any Activity callbacks, then PKCS#11 follows the Activity lifecycle and a blocking `C_WaitForSlotEvent` listener drives token presence. Master public-key derivation also mirrors the vendor `DerivationPath()` representation: a nullable empty path, not a non-null zero-length JNA allocation. The user-facing wallet backend remains simulated until physical validation and provisioning are complete. UI strings are in Russian; tests assert on those exact strings.
...
to `Application.onCreate`, and v1.44 passes the vendor-required null pointer for the empty master derivation path.
