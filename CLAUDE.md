...
`docs/development-plan.md` is the canonical roadmap and source of truth for scope and phase status. Phases 0–7 are complete (Phase 7 = the simulated external-device **custody** foundation; no real NFC/SDK). Phase 9 — making this a **wallet-side** wallet that *receives* WalletConnect v2 / AirGap signing requests — is **feature-complete**: the real `reown_walletkit` impl is wired (device-validated on iOS sim for connect/disconnect, `personal_sign`, and the transaction approve+sign flow; a live on-chain broadcast with test funds is the only remaining WalletConnect check), with inbound signing of transactions + `personal_sign`/`eth_sign` (EIP-191) + EIP-712 typed-data over both WalletConnect and AirGap, a Connections screen, and QR input by file (all platforms) + camera (Android/iOS). The earlier Phase 8 **payload codecs** survive and are reused; its outbound direction (the app *requesting* a signature from an external signer) was the wrong role and was removed.
...
The wallet is the side that *receives* signing requests. Pure-serialization payload codecs (no relay/SDK): `WalletConnectV2RequestCodec` (`walletconnect/wallet_connect_v2.dart`) maps a prepared transfer ↔ an `eth_signTransaction` request, decodes a signed-tx-hex response, recognises/decodes `personal_sign`/`eth_sign` + `eth_signTypedData_v4`/`_v3`, and validates `wallet_switchEthereumChain` for the built-in Mainnet/Sepolia catalog; `AirGapPayloadCodec` (`airgap/airgap_signing.dart`) encodes/decodes the offline QR payloads with request-id validation; `walletconnect/eip712.dart` (`Eip712Encoder`) does pure-Dart EIP-712 hashing. `walletconnect/wallet_connect_service.dart` is the `WalletConnectService` seam (pair → proposal → approve → requests → respond), with `UnavailableWalletConnectService` as the desktop/test-host default and `FakeWalletConnectService` for tests. The **real** impl is `ReownWalletConnectService` (`walletconnect/reown_wallet_connect_service.dart`) over `reown_walletkit`, DI-selected on Android/iOS when `WC_PROJECT_ID` is set. The controller queues inbound requests in arrival order and serializes handling; signing requests go through the summary-bound active backend, while supported chain-switch requests respond with JSON null without unlocking the vault. The inbound request→vault-sign flow is `WalletConnectInboundCoordinator` (`walletconnect/wallet_connect_inbound.dart`: decode → sign via the active backend's `WalletTransactionSigner` → broadcast/respond; every path responds so the dApp never hangs); AirGap's offline equivalent is `AirGapInboundCoordinator`. The UI is the `connections` stage (`wallet_flow_screen_connections.dart`). There is no outbound "request a signature from an external signer" path — that was removed as the wrong role.
