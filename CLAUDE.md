...
The wallet is the side that *receives* signing requests. Pure-serialization payload codecs (no relay/SDK): `WalletConnectV2RequestCodec` (`walletconnect/wallet_connect_v2.dart`) maps a prepared transfer ↔ an `eth_signTransaction` request, decodes a signed-tx-hex response, and recognises/decodes `personal_sign`/`eth_sign` + `eth_signTypedData_v4`/`_v3`; `AirGapPayloadCodec` (`airgap/airgap_signing.dart`) encodes/decodes the offline QR payloads with request-id validation; `walletconnect/eip712.dart` (`Eip712Encoder`) does pure-Dart EIP-712 hashing. `walletconnect/wallet_connect_service.dart` is the `WalletConnectService` seam (pair → proposal → approve → requests → respond), with `UnavailableWalletConnectService` as the desktop/test-host default and `FakeWalletConnectService` for tests. The **real** impl is `ReownWalletConnectService` (`walletconnect/reown_wallet_connect_service.dart`) over `reown_walletkit`, DI-selected on Android/iOS when `WC_PROJECT_ID` is set. The inbound request→vault-sign flow is `WalletConnectInboundCoordinator` (`walletconnect/wallet_connect_inbound.dart`: decode → sign via the active backend's `WalletTransactionSigner` → broadcast/respond; every path responds so the dApp never hangs); AirGap's offline equivalent is `AirGapInboundCoordinator`. The UI is the `connections` stage (`wallet_flow_screen_connections.dart`). There is no outbound "request a signature from an external signer" path — that was removed as the wrong role.
