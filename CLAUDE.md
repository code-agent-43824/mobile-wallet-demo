...
Flutter demo of a mobile EVM crypto wallet targeting Android, iOS, and Windows x64. It supports the full onboarding/auth shell, an encrypted on-device key vault, read-only blockchain access (Ethereum Mainnet + Sepolia), and end-to-end EIP-1559 signing/sending. Android v1.41 includes the first real Rutoken NFC/PKCS#11 transport and a read-only physical diagnostic; the user-facing wallet backend remains simulated until physical validation and provisioning are complete. UI strings are in Russian; tests assert on those exact strings.
...
legacy/EIP-1559/message/AirGap output. v1.41 adds the official Android PC/SC + PKCS#11/JNA stack, Kotlin
platform-channel adapter, and a non-mutating physical transport probe. Physical Android validation, recoverable
