...
`docs/development-plan.md` is the canonical roadmap and source of truth for scope and phase status. Phases 0–7 are complete (Phase 7 = the simulated external-device **custody** foundation; no real NFC/SDK). Phase 9 — making this a **wallet-side** wallet that *receives* WalletConnect v2 / AirGap signing requests — is **feature-complete**: the real `reown_walletkit` impl is wired and device-validated on Android through a confirmed Sepolia broadcast, with inbound signing of transactions + `personal_sign`/`eth_sign` (EIP-191) + EIP-712 typed-data over both WalletConnect and AirGap, a Connections screen, and QR input by file (all platforms) + camera (Android/iOS). v1.37 adds EIP-5792 capability discovery plus live simulation/gas/fee preflight for contract calls. The earlier Phase 8 **payload codecs** survive and are reused; its outbound direction (the app *requesting* a signature from an external signer) was the wrong role and was removed.
...
### WalletConnect v2 / AirGap (Phase 9, wallet-side inbound)

The wallet is the side that *receives* signing requests. Pure-serialization payload codecs (no relay/SDK): `WalletConnectV2RequestCodec` (`walletconnect/wallet_connect_v2.dart`) maps a prepared transfer ↔ an `eth_signTransaction` request, decodes a signed-tx-hex response, recognises/decodes `personal_sign`/`eth_sign` + `eth_signTypedData_v4`/`_v3`, and handles `wallet_switchEthereumChain` plus EIP-5792 `wallet_getCapabilities` for the built-in Mainnet/Sepolia catalog; `AirGapPayloadCodec` (`airgap/airgap_signing.dart`) encodes/decodes the offline QR payloads with request-id validation; `walletconnect/eip712.dart` (`Eip712Encoder`) does pure-Dart EIP-712 hashing. `walletconnect/wallet_connect_service.dart` is the `WalletConnectService` seam (pair → proposal → approve → requests → respond), with `UnavailableWalletConnectService` as the desktop/test-host default and `FakeWalletConnectService` for tests. The **real** impl is `ReownWalletConnectService` (`walletconnect/reown_wallet_connect_service.dart`) over `reown_walletkit`, DI-selected on Android/iOS when `WC_PROJECT_ID` is set; it rejects unsupported required namespaces and filters unsupported optional methods/chains. The controller queues signing requests in arrival order, auto-answers capability probes without UI/PIN, and serializes handling through the summary-bound active backend. `PublicRpcWalletConnectTransactionPreflight` executes `eth_call` before auth, fills omitted gas/EIP-1559 fees from live RPC, and creates the request-bound contract/value/calldata/max-fee preview reused by `WalletConnectInboundCoordinator` for signing and broadcast. AirGap's offline equivalent is `AirGapInboundCoordinator`. The UI is the `connections` stage (`wallet_flow_screen_connections.dart`). There is no outbound "request a signature from an external signer" path — that was removed as the wrong role.
