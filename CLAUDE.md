...
`docs/development-plan.md` is the canonical roadmap and source of truth for scope and phase status. Phases 0–7 are complete (Phase 7 = the simulated external-device **custody** foundation; no real NFC/SDK). Phase 9 — making this a **wallet-side** wallet that *receives* WalletConnect v2 / AirGap signing requests — is in progress: the inbound `WalletConnectService` seam + a fake exist, but no relay/SDK is wired yet. Only the WC v2 / AirGap **payload codecs** survive from the earlier Phase 8 work; its outbound direction (the app *requesting* a signature from an external signer) was the wrong role and was removed.
...
### WalletConnect v2 / AirGap (Phase 9, wallet-side inbound — foundation only)

The wallet is the side that *receives* signing requests. Two payload codecs survive from earlier work and are pure serialization (no relay/SDK): `WalletConnectV2RequestCodec` (`walletconnect/wallet_connect_v2.dart`) maps a prepared transfer ↔ an `eth_signTransaction` request and decodes a signed-tx-hex response; `AirGapPayloadCodec` (`airgap/airgap_signing.dart`) encodes/decodes the offline export/import QR payloads with request-id validation. `walletconnect/wallet_connect_service.dart` adds the wallet-side `WalletConnectService` seam (pair → session proposal → approve → incoming requests → respond), with `UnavailableWalletConnectService` as the shippable default and `FakeWalletConnectService` for tests. The real `reown_walletkit` impl, the inbound request→vault-sign flow, and a connections screen are **not built yet**. There is no outbound "request a signature from an external signer" path — that was removed as the wrong role.
