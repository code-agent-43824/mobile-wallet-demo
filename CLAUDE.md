...
Flutter demo of a mobile EVM crypto wallet targeting Android, iOS, and Windows x64. It supports the full onboarding/auth shell, an encrypted on-device key vault, read-only blockchain access (Ethereum Mainnet + Sepolia), and end-to-end EIP-1559 signing/sending. Android v1.48 includes a physically validated Rutoken NFC/PKCS#11 read/sign transport and recoverable provisioning, plus production backend registration for the shared own-send, WalletConnect, and AirGap signer seam. Startup restores only the software-retained public Rutoken profile without NFC; each private operation opens one fresh card/PIN session. The native transport startup mirrors the official demo: the PC/SC bridge attaches from a custom `Application.onCreate` before any Activity callbacks, then PKCS#11 follows the Activity lifecycle and a blocking `C_WaitForSlotEvent` listener drives token presence. The native crypto boundary is intentionally minimal and follows the supplied example: import one raw BIP32 master private key + chain code with `C_CreateObject`, find it, derive a public address key or per-operation EC private child, call raw `CKM_ECDSA`, destroy transient children, and close the session. It does not query derived chain code or xpub. EC-point intake accepts/validates both compressed and uncompressed SEC1 values with or without the PKCS#11 DER OCTET STRING wrapper. UI strings are in Russian; tests assert on those exact strings.
...
the secret-free transient device signer. Physical signing-matrix dogfood and the Swift implementation are next.
See the NOW / NEXT
