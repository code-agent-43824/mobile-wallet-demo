...
Flutter demo of a mobile EVM crypto wallet targeting Android, iOS, and Windows x64. It supports the full onboarding/auth shell, an encrypted on-device key vault, read-only blockchain access (Ethereum Mainnet + Sepolia), and end-to-end EIP-1559 signing/sending. Android v1.45 includes the first real Rutoken NFC/PKCS#11 transport and a read-only physical diagnostic. Its startup mirrors the official demo: the PC/SC bridge attaches from a custom `Application.onCreate` before any Activity callbacks, then PKCS#11 follows the Activity lifecycle and a blocking `C_WaitForSlotEvent` listener drives token presence. Master public-key derivation mirrors the vendor nullable empty path, and EC-point intake accepts/validates both compressed and uncompressed SEC1 values with or without the PKCS#11 DER OCTET STRING wrapper. The user-facing wallet backend remains simulated until physical validation and provisioning are complete. UI strings are in Russian; tests assert on those exact strings.
