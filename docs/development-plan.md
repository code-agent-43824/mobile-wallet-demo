...
Status: ⏳ In progress (option A — build on the fake, defer the real SDK). 9.0–9.1 done (v1.18); 9.3 done on the fake; **9.4 done** (9.4a DI + controller WC seam; 9.4b Connections screen, v1.20; 9.4c incoming-request approval sheet → `WalletConnectInboundCoordinator`, v1.21), and 9.5 (AirGap inbound: `AirGapInboundCoordinator` decode→sign→response payload + Connections-screen section, v1.22). 9.2 (real `reown_walletkit`) deferred behind native-build blockers; next is camera QR (scan) for WC pairing + AirGap, and `personal_sign`/typed-data.
