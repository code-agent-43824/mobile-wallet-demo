...
- ✅ Phase 9 (real **wallet-side** inbound signing — WalletConnect v2 + AirGap — plus a connections screen and an incoming-request approval flow) is **feature-complete** (chunks 9.0–9.9): real `reown_walletkit` wired and **device-validated** for connect/disconnect/`personal_sign` (9.2), inbound signing of tx + `personal_sign`/`eth_sign` + EIP-712 typed-data, QR by file (all platforms) + camera (Android/iOS). Only owner-pending item: **WalletConnect v2 is device-validated on iOS sim** (connect/disconnect, `personal_sign`, transaction-approve + the per-op PIN flow); the one remaining check is **AirGap inbound**, now via the MetaMask-compatible EIP-4527 / BC-UR path (Phase 12, in progress) — see the "Phase 9" / "Phase 12" sections below
...
Status: ✅ **Feature-complete (chunks 9.0–9.9).** 9.0–9.1 done (v1.18); 9.3 done; **9.4 done** (9.4a DI + controller WC seam; 9.4b Connections screen, v1.20; 9.4c incoming-request approval sheet → `WalletConnectInboundCoordinator`, v1.21); 9.5 (AirGap inbound, v1.22); **9.6 done** (the `QrScanner` seam + sheet polish, v1.23; real **all-platform QR load from an image file** — `FileQrScanner` via `file_selector` + `image` + `zxing2` — v1.24); **9.7 done** (message signing: `personal_sign` / `eth_sign`, EIP-191, v1.25); **9.8 done** (EIP-712 `eth_signTypedData_v4`/`_v3` via a pure-Dart `Eip712Encoder` + `signDigest`, v1.26); **9.2 done** (real `ReownWalletConnectService` over `reown_walletkit`, v1.27 — the 9.2a probe + 9.2b-i toolchain fixes cleared the native blockers; **device-validated** for connect/disconnect/`personal_sign` by owner dogfood 2026-06-16); **9.9 done** (live **camera** QR scan via `mobile_scanner` on Android/iOS, v1.28; scan-window overlay + torch, v1.29). The only remaining item is the owner's **AirGap inbound** end-to-end check (needs a MetaMask / air-gapped companion setup) — WalletConnect v2 is now **device-validated on iOS sim** (owner dogfood, v1.33): connect/disconnect, `personal_sign`, and transaction-approve, plus the Phase 11 per-op PIN flow (a silent-approve gap was fixed in v1.33). See "Pending owner verification" below.

### Pending owner verification
- **AirGap inbound, end-to-end** (TODO, owner) — **now via the MetaMask-compatible EIP-4527 / BC-UR path (Phase 12)**, not the original `airgap-tx:` format. The legacy `airgap-tx:` codec/coordinator (chunk 9.5) is unit-tested only, was never device-verified (owner hadn't set up MetaMask as of 2026-06-16), and is **superseded** — removed in chunk 12.5. The EIP-4527 codec + inbound sign path + account export (12.1–12.3) are CI-green via unit tests + offline derivation; the device dogfood (add the app to MetaMask as a QR-based hardware wallet → sign a tx/message → broadcast) happens once **12.4** wires the AirGap UI. Both directions reuse the same vault sign path WalletConnect already exercises on device.
