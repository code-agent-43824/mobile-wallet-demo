...
Status: ⏳ In progress (option A — build on the fake, defer the real SDK). 9.0–9.1 done (v1.18); 9.3 done on the fake; **9.4 done** (9.4a DI + controller WC seam; 9.4b Connections screen, v1.20). 9.2 (real `reown_walletkit`) deferred behind native-build blockers; next is the incoming-request approval sheet + AirGap inbound + QR pairing.
...
- **9.4c** — incoming-request approval sheet: subscribe the controller to `WalletConnectService.requests`, surface a request sheet (details → approve/reject), and drive `WalletConnectInboundCoordinator` (sign via the active backend → broadcast/respond) on approval.
