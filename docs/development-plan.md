...
- **AirGap inbound, end-to-end** (TODO, owner) — scan/paste an `airgap-tx:` request from an air-gapped companion (e.g. MetaMask's AirGap export, or a real offline signer), sign it offline in the Connections → AirGap section, return the `airgap-sig:` response, and confirm the companion accepts it. Not yet tested on device (owner hadn't set up MetaMask as of 2026-06-16). The codec + coordinator are covered by unit tests and the same vault sign path WalletConnect already uses on device.
