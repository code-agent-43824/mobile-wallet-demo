...
- AirGap: a QR scanner (e.g. `mobile_scanner`) for the offline round-trip (camera).
...
- **9.6** — QR scan for WalletConnect pairing + AirGap + incoming-request sheet polish. ✅ done (v1.23): a `QrScanner` seam (`qr/qr_scanner.dart` — interface + `UnavailableQrScanner` default + `FakeQrScanner`) injected through `MobileWalletDemoApp` → controller (`scanQrCode` / `isQrScannerAvailable`); gated "Сканировать wc: URI" / "Сканировать airgap-tx" entry points that fill the paste fields; request-card polish (sender line). Seam + controller + widget tests. *The real camera impl (`mobile_scanner`) is **deferred** — it doesn't support Windows x64 (a CI build target) and needs per-platform camera permissions; like 9.2's real SDK, it lands when native platforms are tackled. Paste remains the fallback.*
...
- [x] QR-code pairing for WalletConnect (`QrScanner` seam + gated scan entry points, v1.23; real `mobile_scanner` camera deferred)
- [ ] tests + docs
...
- `v1.23` — Phase 9 chunk 9.6: the `QrScanner` seam (`qr/qr_scanner.dart`: interface + `UnavailableQrScanner` default + `FakeQrScanner`) injected through `MobileWalletDemoApp` → `WalletFlowController` (`scanQrCode`/`isQrScannerAvailable`); gated "Сканировать" entry points on the Connections screen that fill the `wc:`/`airgap-tx:` paste fields; request-card sender line. Scanner + controller + widget tests. Real `mobile_scanner` camera deferred (no Windows support + camera permissions)
