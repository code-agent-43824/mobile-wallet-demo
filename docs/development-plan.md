...
- ✅ Phase 9 (real **wallet-side** inbound signing — WalletConnect v2 + AirGap — plus a connections screen and an incoming-request approval flow) is **feature-complete** (chunks 9.0–9.9): real `reown_walletkit` wired and **device-validated** for connect/disconnect/`personal_sign` (9.2), inbound signing of tx + `personal_sign`/`eth_sign` + EIP-712 typed-data, QR by file (all platforms) + camera (Android/iOS). Only owner **transaction** dogfood (pending test funds) remains — see the "Phase 9" section below
