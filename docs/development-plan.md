...
- **NOW — v1.43.0+54:** phone-vault custody, Mainnet/Sepolia reads and sends, wallet-side WalletConnect,
...
is present in v1.43 and awaits repeat physical validation; provisioning, production-backend wiring, full dogfood, and iOS remain.
...
- **10.0 — IMPLEMENTED, PHYSICAL RETEST PENDING (v1.43):** the exact Android v1.1 artifacts are vendored with
...
- **10.3 — IMPLEMENTED, PHYSICAL RETEST PENDING (v1.43):** official rtpcscbridge 1.4.0 + pkcs11wrapper 4.3.1 +
  ...
  Activity lifecycle callback. Retest discovery, then
