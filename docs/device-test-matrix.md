...
| Rutoken custody/NFC discovery | Android owner device | v1.41 / 2026-07-22 | FAIL | Official demo detects the same token on the same phone; Wallet Demo timed out because it polled the slot list without the vendor `C_WaitForSlotEvent` listener. Fix is in v1.42; owner retest pending. |
...
| Check | Android | iOS | Acceptance |
| --- | --- | --- | --- |
| Vendor stack init, token discovery, login, public-key read, teardown | RETEST v1.42 | BLOCKED | One-tap lifecycle works without custom APDU handling. v1.41 discovery failed; v1.42 mirrors the official slot-event listener. |
